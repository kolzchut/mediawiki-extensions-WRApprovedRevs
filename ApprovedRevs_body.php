...
class ApprovedRevs {
	...
	private static $mProjectForPage = array();
	private static $mGroupForPage = array();
	private static $mOrganizationForPage = array();
	private static $mApprovedRevIdForPage = array();

	private static $mApprovalStatusForPage = array();
	...
	public static function getProjectName( Title $title ) {
		...
		$projectName = $dbr->selectField(
			'approved_pages', 'ap_project', array( 'ap_page_id' => $pageID )
		);
	...
	public static function getOrganizationName( Title $title ) {
		...
		$dbr = wfGetDB( DB_SLAVE );
		$organizationName = $dbr->selectField(
			'approved_pages', 'ap_organization', array( 'ap_page_id' => $pageID )
		);
	...
	public static function getGroup( Title $title ) {
		...
		$dbr = wfGetDB( DB_SLAVE );
		$userGroup = $dbr->selectField(
			'approved_pages', 'ap_user_group', array( 'ap_page_id' => $pageID )
		);
	...
	public static function getApprovalStatus( Title $title ) {
		...
		$approvalStatus[ 'time' ] = $dbr->selectField(
			'approved_pages', 'ap_review_timestamp', array( 'ap_page_id' => $pageID )
		);
		$approvalStatus[ 'on_behalf' ] = $dbr->selectField(
			'approved_pages', 'ap_review_on_behalf', array( 'ap_page_id' => $pageID )
		);
		$approvalStatus[ 'on_behalf_text' ] = $dbr->selectField(
			'approved_pages', 'ap_review_on_behalf_comments', array( 'ap_page_id' => $pageID )
		);
	...
	public static function saveProjectAssociationInDB(
		Title $title, $action, $project, $organization, $group
	) {
	...
	public static function logProjectAssignment(
		Title $title, $action, User $user, $project, $organization, $group, $reason = null
	) {
		...
		$projectName = self::isAssignedToProject( $title ) ?
			self::getProjectName( $title ) : $project;
	...
	public static function logPageApproval( Title $title, User $user, $on_behalf, $comments ) {
		...
		if ( $action == 'approve' && !empty( $comments ) ) {
			$action = 'approvewithcomment';
		}
		$logEntry = new ManualLogEntry( 'approvedrevs', $action );
	...
	public static function getRevStatusMsg( Title $title ) {
		...
		$approved_rev_link = Linker::link(
			$title, 'לחצו לגרסה האחרונה שאושרה', array(), array( 'oldid' => $approved_rev_id )
		);
